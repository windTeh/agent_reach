...
import { BrowserCommandError, clearDaemonRunContext, fetchDaemonStatus, getDaemonHealth, isUnknownOutcomeError, requestDaemonShutdown, sendCommand, setDaemonCommandTimeoutSeconds, setDaemonRunContext, } from './daemon-client.js';
...
describe('daemon-client', () => {
    ...
    it('clearDaemonRunContext only clears the context that still belongs to the runId', async () => {
        ...
        await sendCommand('exec', { code: '1 + 1', surface: 'adapter', session: 'site:chatgpt', siteSession: 'persistent' });
        const kept = JSON.parse(String(vi.mocked(fetch).mock.calls[0][1]?.body));
        expect(kept.runId).toBe('run_4242_1_a');
        clearDaemonRunContext('run_4242_1_a');
        await sendCommand('exec', { code: '2 + 2', surface: 'adapter', session: 'site:chatgpt', siteSession: 'persistent' });
        const cleared = JSON.parse(String(vi.mocked(fetch).mock.calls[1][1]?.body));
    ...
    it('throws a terminal SessionBusyError on a session_busy response without retrying', async () => {
        ...
        await expect(sendCommand('exec', { code: '1 + 1', surface: 'adapter', session: 'site:chatgpt', siteSession: 'persistent' })).rejects.toBeInstanceOf(SessionBusyError);
    ...
    it('sendCommand forwards the config default as a soft preferredContextId, not a requirement', async () => {
        ...
        fs.writeFileSync(path.join(configDir, 'browser-profiles.json'), JSON.stringify({ version: 1, aliases: {}, defaultContextId: 'zvypsyje' }));
