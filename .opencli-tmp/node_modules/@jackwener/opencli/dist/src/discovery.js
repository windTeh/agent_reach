...
const warnedYamlAdapterPaths = new Set();
const cliModuleLoadResults = new Map();
...
async function loadFromManifest(manifestPath, clisDir) {
    try {
        ...
        for (const entry of manifest) {
            ...
            if (entry.sourceFile)
                manifestFiles.add(path.resolve(clisDir, entry.sourceFile));
            const cmd = {
                site: entry.site,
                name: entry.name,
...
async function warnIgnoredYamlAdaptersInCliRoot(dir, manifestFiles) {
    let entries;
    try {
...
async function warnIgnoredYamlAdaptersInFlatDir(dir, options) {
    let files;
    try {
    ...
    for (const file of files.filter(isYamlAdapterFile).sort()) {
        const filePath = path.join(dir, file);
        if (!(await isLikelyYamlAdapter(filePath, options.pluginMode === true)))
            continue;
        if (await hasUsableJsReplacement(dir, file, fileSet, options.manifestFiles))
            continue;
        const warningKey = await stableWarningKey(filePath);
        if (warnedYamlAdapterPaths.has(warningKey))
            continue;
    ...
    if (ignored.length === 0)
        return;
    log.warn(`Ignoring ${ignored.length} YAML adapter${ignored.length === 1 ? '' : 's'} for ${options.owner}: `
        + `${ignored.join(', ')}. .yaml/.yml adapters are no longer loaded; convert them to .js with the cli() API.`);
}
function isYamlAdapterFile(file) {
    return YAML_ADAPTER_EXT_RE.test(file);
}
async function hasUsableJsReplacement(dir, yamlFile, fileSet, manifestFiles) {
    const jsFile = yamlFile.replace(YAML_ADAPTER_EXT_RE, '.js');
    if (!fileSet.has(jsFile))
        return false;
    const jsPath = path.resolve(dir, jsFile);
    if (manifestFiles)
        return manifestFiles.has(jsPath);
    return loadCliModule(jsPath);
}
async function isLikelyYamlAdapter(filePath, pluginMode) {
    let source;
    try {
...
async function stableWarningKey(filePath) {
    try {
...
async function cliModuleCacheKey(filePath) {
...
async function loadCliModule(filePath) {
