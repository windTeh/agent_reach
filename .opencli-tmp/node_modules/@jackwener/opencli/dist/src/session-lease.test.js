...
import { SESSION_LEASE_TTL_MS, SessionLeaseRegistry, buildSessionBusyFailure, getSessionLeaseKey, isSessionLeaseCommand, parsePidFromRunId, } from './session-lease.js';
const T0 = 1_000_000;
function writeCommand(over = {}) {
    return {
        surface: 'adapter',
...
describe('buildSessionBusyFailure', () => {
    it('names the holder, its pid, and how long it has held the lease', () => {
        const failure = buildSessionBusyFailure('site:chatgpt', { runId: 'run_111_1_a', command: 'chatgpt ask', pid: 111, startedAt: T0, lastSeenAt: T0 + 40_000 }, T0 + 42_000);
    ...
    it('degrades gracefully when the pid is unknown', () => {
        const failure = buildSessionBusyFailure('site:chatgpt', { runId: 'x', command: 'chatgpt ask', pid: null, startedAt: T0, lastSeenAt: T0 }, T0 + 5_000);
