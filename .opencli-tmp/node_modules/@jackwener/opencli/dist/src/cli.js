...
function collectRepoSharedDependencies(entryDir, sharedDir) {
    ...
    for (let index = 0; index < pending.length; index += 1) {
        ...
        for (const specifier of extractImportSpecifiers(source)) {
            ...
            if (!resolved || !isPathInside(resolved, sharedDir))
                continue;
            sharedDeps.add(resolved);
            if (fs.existsSync(resolved) && fs.statSync(resolved).isFile())
                pending.push(resolved);
            else if (fs.existsSync(resolved) && fs.statSync(resolved).isDirectory())
                pending.push(...listJsFiles(resolved));
        }
    }
    return [...sharedDeps].sort();
}
function copyEjectedRepoSharedDependencies(builtinSiteDir, builtinSharedDir, userClisDir) {
