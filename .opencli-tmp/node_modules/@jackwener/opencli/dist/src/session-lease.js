...
export class SessionLeaseRegistry {
    ttlMs;
    leases = new Map();
    constructor(ttlMs = SESSION_LEASE_TTL_MS) {
        this.ttlMs = ttlMs;
    }
    /**
    ...
    touch(key, input) {
        ...
        const alive = current !== undefined && (input.now - current.lastSeenAt <= this.ttlMs || input.hasPendingWork?.(current.runId) === true);
        if (current !== undefined && alive && current.runId !== input.runId) {
            return { granted: false, holder: current };
        }
        const holder = current !== undefined && current.runId === input.runId
...
export function buildSessionBusyFailure(session, holder, now) {
