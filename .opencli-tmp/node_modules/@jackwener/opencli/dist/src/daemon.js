...
import { PROFILE_DISCONNECTED_HINT, buildCommandDispatchFailure, buildCommandTimeoutFailure, buildExtensionDisconnectFailure, getResponseCorsHeaders, resolveProfileRoute, } from './daemon-utils.js';
import { SessionLeaseRegistry, buildSessionBusyFailure, getSessionLeaseKey, isSessionLeaseCommand, } from './session-lease.js';
...
function runHasPendingWork(runId) {
...
function settlePending(id, entry, outcome) {
...
const staleDefaultWarned = new Set();
function resolveExtensionConnection(contextId, preferredContextId) {
    ...
    if (route.fallbackFrom && !staleDefaultWarned.has(route.fallbackFrom)) {
        ...
        log.warn(`[daemon] Default profile "${route.fallbackFrom}" is not connected; ` +
        ...
        try {
            ...
            if (body.action === 'lease-release') {
                if (typeof body.runId === 'string')
                    sessionLeases.releaseByRunId(body.runId);
                jsonResponse(res, 200, { id: body.id, ok: true });
                return;
            }
            const route = resolveExtensionConnection(typeof body.contextId === 'string' ? body.contextId : undefined, typeof body.preferredContextId === 'string' ? body.preferredContextId : undefined);
            if (!route.connection) {
            ...
            if (isSessionLeaseCommand(body)) {
                ...
                if (!outcome.granted) {
                    ...
                    log.warn(`[daemon] Session ${key} busy — rejected ${body.command ?? body.action} ` +
...
function shutdown() {
    ...
    for (const [id, p] of pending) {
        const failure = p.dispatched
            ? new DaemonCommandFailure('Daemon shutting down before the command completed.', 'daemon_shutting_down', 'The daemon is being replaced; a journaling extension replays the command result on retry.', 503)
            : (() => {
