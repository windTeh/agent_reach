...
function sanitizeContext(obj) {
    ...
    try {
        ...
        const jsonStr = JSON.stringify(obj, (_key, value) => typeof value === 'bigint' ? value.toString() : value);
        _sanitizeCache.set(objRef, jsonStr);
        return JSON.parse(jsonStr);
    }
    catch (err) {
        log.warn(`[pipeline/template] sanitizeContext failed: ${err instanceof Error ? err.message : String(err)}. ` +
