...
import { detectAppProcess, detectProcess, discoverAppPath, electronLaunchArgs, findAppProcessPids, killAppProcess, launchDetachedApp, launchElectronApp, probeCDP, resolveExecutableCandidates, } from './launcher.js';
...
describe('discoverAppPath', () => {
    ...
    it.skipIf(process.platform !== 'darwin')('uses bundle id Spotlight fallback when osascript cannot resolve the display name', () => {
        ...
        expect(cp.execFileSync).toHaveBeenNthCalledWith(1, 'osascript', ['-e', 'POSIX path of (path to application id "com.openai.codex")'], { encoding: 'utf-8', stdio: 'pipe', timeout: 5_000 });
        expect(cp.execFileSync).toHaveBeenNthCalledWith(2, 'mdfind', ['kMDItemCFBundleIdentifier == "com.openai.codex"'], { encoding: 'utf-8', stdio: 'pipe', timeout: 5_000 });
    });
...
describe.skipIf(process.platform === 'win32')('app-scoped process detection', () => {
    ...
    function mockProcessTable(rows) {
...
describe('launchElectronApp', () => {
    ...
    it('does not fall back to the next executable when the first candidate fails for a non-ENOENT reason', async () => {
        ...
        await expect(launchElectronApp('/Applications/Codex.app', app, ['--remote-debugging-port=9238'], 'Codex')).rejects.toThrow('Failed to launch Codex');
    ...
    it('throws a typed all-candidates-missing error when every executable candidate is absent', async () => {
        ...
        await expect(launchElectronApp('/Applications/MissingCodex.app', app, ['--remote-debugging-port=9238'], 'Codex')).rejects.toThrow('Could not launch Codex: no compatible executable found');
