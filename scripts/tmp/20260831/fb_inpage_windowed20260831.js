...
(() => {
  ...
  const extractRow = (node) => {
    ...
    const owner = ownerInfo ? (ownerInfo.title || ownerInfo.id) : null;
    // Account ID: owner.id 形如 "GraphQLTofuIGAccountEntityInfo:17841406045865168"，去前缀取纯数字
    const account_id = ownerInfo ? String(ownerInfo.id || '').replace(/^GraphQLTofuIGAccountEntityInfo:/, '') : null;
    const account_username = ownerInfo ? (ownerInfo.title || null) : null;
    ...
    return {
      ...
      // 扩展字段: 能拿到的 + 预留（Content API 无 video_id/profile_visits/navigation）
//      account_id, account_username,
//      video_id: '', profile_visits: null, navigation: null,
    };
  };
