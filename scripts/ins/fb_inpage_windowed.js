...
(() => {
  ...
  const extractRow = (node) => {
    ...
    // account_username：账号用户名（原 owner 字段，Content API 返回 owner.title）
    ...
    return {
      ...
      // 内部字段：owner.id 去前缀的纯数字账号 ID，供 account_name 映射表 fallback 查 key
      _owner_id: ownerInfo ? String(ownerInfo.id || '').replace(/^GraphQLTofuIGAccountEntityInfo:/, '') : '',
      // 媒体级字段（账号显示名 / 视频时长ms / 账号fbid / 媒体类型）：
      // Content 表格接口不返回这些键，取不到时留空；
      // 若将来接入 IG 网页 API / Graph API 等数据源，此处按键提取即自动生效。
      ...
      // 媒体产品类型（clips=reel, feed=photo, story=story 等）；Content 表格接口可能不返回此键
      media_product_type: info.media_product_type || null,
    };
  };
