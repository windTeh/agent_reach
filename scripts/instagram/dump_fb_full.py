...
js = "(function(){return JSON.stringify({len:(window.__fullResp||'').length});})()"
out = ev(js)
d = json.loads(out)
total = d['len']
...
for i in range((total + chunk - 1) // chunk):
    js = "(function(){var r=window.__fullResp||'';var s=r.slice(%d,%d);return JSON.stringify({ok:1,c:s});})()" % (i*chunk, (i+1)*chunk)
    out = ev(js)
    try:
        d = json.loads(out)
        parts.append(d['c'])
    except Exception as e:
        print('chunk', i, 'ERR', str(e), out[:150])
        sys.exit(1)
    if i % 10 == 0: print('chunk', i, 'done', flush=True)

full = ''.join(parts)
with open('fb_20rows_full.json', 'w', encoding='utf-8') as f:
    f.write(full)
